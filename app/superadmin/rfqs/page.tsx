    ...
    <div>
          ...
          <tbody>
            {rows.map(r => (
              <tr key={r.id} className="border-b border-gray-100 hover:bg-gray-50">
                ...
                <Td><Link href={`/superadmin/rfqs/${r.id}`} className="text-blue-600 hover:underline">Ver</Link></Td>
              </tr>
            ))}
            {rows.length === 0 && (
      ...
      {totalPages > 1 && (
        <div className="flex gap-2 mt-4 justify-end">
          {page > 0 && <Link href={`/superadmin/rfqs?page=${page - 1}`} className="px-3 py-1 border rounded text-sm hover:bg-gray-50">← Anterior</Link>}
          <span className="px-3 py-1 text-sm text-gray-500">{page + 1} / {totalPages}</span>
          {page < totalPages - 1 && <Link href={`/superadmin/rfqs?page=${page + 1}`} className="px-3 py-1 border rounded text-sm hover:bg-gray-50">Siguiente →</Link>}
        </div>
      )}
    </div>
  )
