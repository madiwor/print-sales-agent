...
export default async function PortalRFQsPage({
  params,
  searchParams,
}: {
  ...
  const { rows, total } = await listPortalRFQs(slug, page, sp.status)
    ...
    <div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-gray-200 text-gray-500">
              <Th>Fecha</Th><Th>Contacto</Th><Th>Producto</Th><Th>Estado</Th><Th />
            </tr>
          </thead>
          <tbody>
            {rows.map(r => (
              <tr key={r.id} className="border-b border-gray-100 hover:bg-gray-50">
                ...
                <Td><Link href={`/portal/${slug}/admin/rfqs/${r.id}`} className="text-blue-600 hover:underline">Ver</Link></Td>
              </tr>
            ))}
            {rows.length === 0 && (
              <tr><td colSpan={5} className="py-8 text-center text-gray-400">Sin solicitudes aún.</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      {totalPages > 1 && (
        <div className="flex gap-2 mt-4 justify-end">
          {page > 0 && <Link href={`/portal/${slug}/admin/rfqs?page=${page-1}`} className="px-3 py-1 border rounded text-sm hover:bg-gray-50">← Anterior</Link>}
          <span className="px-3 py-1 text-sm text-gray-500">{page+1} / {totalPages}</span>
          {page < totalPages-1 && <Link href={`/portal/${slug}/admin/rfqs?page=${page+1}`} className="px-3 py-1 border rounded text-sm hover:bg-gray-50">Siguiente →</Link>}
        </div>
      )}
    </div>
  )
